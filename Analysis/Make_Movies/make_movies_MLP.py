...
def write_movies(PATH, hdf5file_name, x, y, fps=16):
	...
	with h5py.File(os.path.join(PATH, hdf5file_name), mode="r") as df_total:
		...
		for i in range(df_p.shape[0]):
			...
			cs1 = ax[0].tricontourf(x, y, df_p[i], levels=np.linspace(min_protein_val,max_protein_val,100), cmap='Blues')
			cs2 = ax[1].tricontourf(x, y, df_r[i], levels=np.linspace(min_rna_val,max_rna_val,100), cmap='Reds')
			...
			ax[1].set_title(r'RNA concentration ($\phi_R$)', fontsize=20)
		...
		for file in file_paths:
			clips.append(mp.ImageClip(file).set_duration(0.3))
		
		concat_clip = mp.concatenate_videoclips(clips, method="compose")
		concat_clip.write_videofile(os.path.join(PATH, 'movies','mRNA_lRNA_Protein.mp4'), fps=fps)
		concat_clip.close()
	...
	for root, dirs, files in os.walk(base_path): # Loop through all subdirectories in the directory "base_path"
		...
		for fi in files: # Loop through all files in each subdirectory to see if we have a hdf5 file with the name given in args.f
			...
			if match != None: 		# Found a hdf5 file!
				...
				x = df.values[:,0]
				y = df.values[:,1]
				write_movies(root, args.f, x, y, fps = 16)
